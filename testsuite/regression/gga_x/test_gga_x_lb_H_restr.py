...
def test_gga_x_lb_H_restr_1_vrho():
    # Prepare the input
    ...
    ref_tgt = numpy.asarray([-6.652655970984038e-01, -7.622084968754886e-01, -5.301371105546901e-01, -2.506667529480197e-01, -1.127303815462818e-01])
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05


def test_gga_x_lb_H_restr_1_vsigma():
    # Prepare the input
    inp = test_data["H_restr"]
