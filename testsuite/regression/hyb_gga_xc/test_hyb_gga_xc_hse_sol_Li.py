...
def test_hyb_gga_xc_hse_sol_Li_2_zk():
    # Prepare the input
    inp = test_data["Li"]
    ...
    tgt = out["zk"].flatten()
    ref_tgt = numpy.asarray([-1.402560192086155e+00, -9.971239200154868e-01, -3.056224617182194e-01, -1.476431765052193e-01, -6.933913366963229e-02, -2.045051297531889e-02, -3.838595910663383e-04])
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-08


def test_hyb_gga_xc_hse_sol_Li_2_vrho():
    # Prepare the input
    inp = test_data["Li"]
    ...
    ref_tgt = numpy.asarray([-1.834951064466081e+00, -1.836414083360395e+00, -1.280248633589831e+00, -1.281186124663075e+00, -3.023504834972039e-01, -3.023770440766121e-01, -1.932652892146248e-01, -1.235645631921081e-01, -7.343904729388885e-02, 3.419889428047509e-01, -2.722615686135282e-02, -2.703625496862308e-02, -5.541559468520087e-04, -3.939550388477289e-04])
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05


def test_hyb_gga_xc_hse_sol_Li_2_vsigma():
    # Prepare the input
    inp = test_data["Li"]
