...
def test_hyb_gga_xc_lc_vv10_Li_2_zk():
    # Prepare the input
    inp = test_data["Li"]
    ...
    tgt = out["zk"].flatten()
    ref_tgt = numpy.asarray([-1.608219861651897e+00, -1.082425118427894e+00, -1.953182491157725e-01, -4.398765335352424e-02, -3.707545121271928e-03, -1.430956683197512e-05, -9.778943064584996e-11])
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-08


def test_hyb_gga_xc_lc_vv10_Li_2_vrho():
    # Prepare the input
    inp = test_data["Li"]
    ...
    ref_tgt = numpy.asarray([-2.118445092325595e+00, -2.120440619370029e+00, -1.385859933419040e+00, -1.387119152180498e+00, -1.981259905996673e-01, -1.982648459231216e-01, -7.648273781456226e-02, -9.766543161015308e-02, -1.077864856679737e-02, 3.428185824503964e-01, -2.903051160137719e-05, -2.839855322631591e-05, -2.354492428545516e-10, -8.461654920511137e-11])
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05


def test_hyb_gga_xc_lc_vv10_Li_2_vsigma():
    # Prepare the input
    inp = test_data["Li"]
