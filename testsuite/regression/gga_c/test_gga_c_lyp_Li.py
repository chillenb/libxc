...
def test_gga_c_lyp_Li_2_zk():
    ...
    tgt = out["zk"].flatten()
    ref_tgt = [-5.538903098368998e-02, -4.949790488041998e-02, 1.413408320172361e-02, -1.276112437397293e-05, -2.334131483445219e-09, -2.078188623821984e-03, -3.003770849422545e-05]
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-08


def test_gga_c_lyp_Li_2_vrho():
    # Prepare the input
    inp = test_data["Li"]
    ...
    ref_tgt = [-6.387961131095844e-02, -6.371176627512085e-02, -6.461227879900613e-02, -6.444379768591930e-02, -1.043591405809932e-01, -1.047837406785314e-01, -2.225850205384591e-05, -8.222888742779609e-02, -2.804578703192513e-09, -3.029946428134138e-02, -2.687423197831241e-03, -2.778500684814050e-03, -2.079686215637139e-05, -9.360839371319271e-05]
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05


def test_gga_c_lyp_Li_2_vsigma():
    # Prepare the input
    inp = test_data["Li"]
