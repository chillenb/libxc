...
def test_mgga_xc_vcml_rvv10_H_2_zk():
    ...
    tgt = out["zk"].flatten()
    ref_tgt = [-6.798052096595714e-01, -6.188960936263025e-01, -3.794204710950831e-01, -1.244540780207781e-01, -4.103046651814973e-03]
...
def test_mgga_xc_vcml_rvv10_H_2_vrho():
    ...
    ref_tgt = [-8.994380828871306e-01, 1.721624345323563e+01, -7.929858033223149e-01, 7.103853482010346e+02, -4.549008750805215e-01, 4.161403937222119e+02, -1.754759922527588e-01, 5.429911640212540e+00, -5.499147537289003e-03, 4.283130421726685e-07]
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05
...
def test_mgga_xc_vcml_rvv10_H_2_vtau():
    ...
    ref_tgt = [-2.101633566660515e-12, 0.000000000000000e+00, -4.637777264895696e-11, 0.000000000000000e+00, -2.327003352854420e-10, 0.000000000000000e+00, -1.155765514414988e-15, 0.000000000000000e+00, 9.045008128315109e-06, 0.000000000000000e+00]
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05
