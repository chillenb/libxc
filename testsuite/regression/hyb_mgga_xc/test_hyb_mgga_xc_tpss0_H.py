...
def test_hyb_mgga_xc_tpss0_H_2_zk():
    ...
    tgt = out["zk"].flatten()
    ref_tgt = [-5.282065416703160e-01, -4.740912370458291e-01, -2.779715645815429e-01, -9.858829209444767e-02, -5.547461018925360e-03]
...
def test_hyb_mgga_xc_tpss0_H_2_vrho():
    ...
    ref_tgt = [-6.861516344637013e-01, -6.412806689281922e-02, -6.175928002461044e-01, -3.635674312071321e-02, -3.637165480253491e-01, -1.617448765558878e-02, -9.434833689372316e-02, -3.172051243992244e-04, -7.391414925447553e-03, -4.663822954112051e-10]
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05
...
def test_hyb_mgga_xc_tpss0_H_2_vtau():
    ...
    ref_tgt = [1.760156674157844e+00, -6.182509960461265e+00, 3.094285643695056e-02, -7.616412850374940e-02, -3.642599047454440e-04, -3.228871222511654e-02, -3.729796988339954e-04, -6.238502800849651e-04, -6.466867300223765e-10, -6.808566129057391e-10]
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05
