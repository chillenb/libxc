...
def test_mgga_c_revscan_Li_2_zk():
    # Prepare the input
    inp = test_data["Li"]
    ...
    tgt = out["zk"].flatten()
    ref_tgt = numpy.asarray([-5.000044856604748e-02, -6.848797592369946e-02, -7.815300349796314e-02, -3.793740419020744e-03, -1.859981468143330e-02, 3.425369967853092e-05, 1.436024192532550e-07])
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-08


def test_mgga_c_revscan_Li_2_vrho():
    # Prepare the input
    inp = test_data["Li"]
...
def test_mgga_c_revscan_Li_2_vsigma():
    ...
    ref_tgt = numpy.asarray([1.023824283874695e-04, 2.047648567749390e-04, 1.023824283874695e-04, 7.284036640433407e-04, 1.456807328086681e-03, 7.284036640433407e-04, 3.259469181955202e-01, 6.518938363910406e-01, 3.259469181955202e-01, 5.436504203280996e+00, 1.087300840656199e+01, 5.436504203280996e+00, 2.497453361152924e+02, 4.994906722305848e+02, 2.497453361152924e+02, 1.094243721633776e-02, 2.188487443267553e-02, 1.094243721633776e-02, -1.121841465298950e-03, -2.243682930597899e-03, -1.121841465298950e-03])
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05


def test_mgga_c_revscan_Li_2_vtau():
    # Prepare the input
    inp = test_data["Li"]
