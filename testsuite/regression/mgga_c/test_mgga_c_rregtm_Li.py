...
def test_mgga_c_rregtm_Li_2_zk():
    # Prepare the input
    inp = test_data["Li"]
    ...
    tgt = out["zk"].flatten()
    ref_tgt = numpy.asarray([-2.565506030176055e-02, -2.438854625799005e-02, -1.481541165534285e-02, -2.012273169483131e-04, -3.855018850928288e-08, -1.067648954226573e-03, -5.788917075476808e-06])
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-08


def test_mgga_c_rregtm_Li_2_vrho():
    # Prepare the input
    inp = test_data["Li"]
    ...
    ref_tgt = numpy.asarray([-3.481283186180276e-02, -3.474003143061156e-02, -3.782393253332390e-02, -3.775854944483058e-02, -4.863339842092504e-02, -4.865336083502241e-02, -1.592931098268111e-03, -1.591262284201089e-01, -1.347771748322780e-02, -9.398266853483453e-02, -2.008555855529216e-03, -2.023114096014518e-03, -1.075222067322012e-05, -1.347139221366846e-05])
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05


def test_mgga_c_rregtm_Li_2_vsigma():
    # Prepare the input
    inp = test_data["Li"]
