...
def test_mgga_c_cc_Li_restr_1_vrho():
    ...
    ref_tgt = [-1.025617742488499e-01, -9.247599975071724e-02, -5.666713531650919e-02, -3.908979809355456e-02, -2.363143054234018e-02, -8.569322884269131e-03, -2.365942116388912e-04]
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05
...
def test_mgga_c_cc_Li_restr_1_vlapl():
    # Prepare the input
    inp = test_data["Li_restr"]
