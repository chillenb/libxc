...
def test_mgga_c_rscan_H_2_zk():
    ...
    tgt = out["zk"].flatten()
    ref_tgt = [-7.424616477180734e-16, -9.610368056911511e-16, -4.019354293838262e-15, -8.633718739936569e-14, -1.955045359543911e-11]
...
def test_mgga_c_rscan_H_2_vrho():
    ...
    ref_tgt = [-7.632783294297951e-17, -2.198763077912139e-01, -1.561251128379126e-16, -2.061881205506179e-01, -1.415534356397075e-15, -1.731868428931188e-01, -6.860831347488272e-14, -7.071811968129105e-02, -1.895111931965454e-11, -1.674245713528410e-03]
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05
...
def test_mgga_c_rscan_H_2_vlapl():
    # Prepare the input
    inp = test_data["H"]
...
def test_mgga_c_rscan_H_2_vtau():
    ...
    ref_tgt = [-1.820245022231922e-21, -1.798905354343978e-21, -5.922933159054752e-19, -5.826382676070761e-19, -4.866122583553658e-18, -4.848441818732104e-18, -2.761328545051615e-22, -2.761257011808913e-22, -3.370901254858265e-27, -3.370901281139636e-27]
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05
