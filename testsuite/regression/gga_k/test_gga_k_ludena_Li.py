...
def test_gga_k_ludena_Li_2_zk():
    ...
    tgt = out["zk"].flatten()
    ref_tgt = numpy.asarray([4.217372862378161e+00, 4.099882256383768e+00, 3.215767055237889e+00, 2.108661543542079e-02, 7.165935024025777e-02, 3.084472388730704e+00, 1.356897369842305e+00])
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-08


def test_gga_k_ludena_Li_2_vrho():
    # Prepare the input
    inp = test_data["Li"]
    ...
    ref_tgt = numpy.asarray([-4.217419202003024e+00, -4.217326647787661e+00, -4.098796785522157e+00, -4.100964960284973e+00, -3.210976564408328e+00, -3.220563663050525e+00, -2.007652420859430e-02, -3.057065625943076e+00, -7.165915626259617e-02, -1.210227402337043e+00, -3.032148342681578e+00, -3.137954916182148e+00, -1.418447087986350e+00, -1.185586587718588e+00])
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05


def test_gga_k_ludena_Li_2_vsigma():
    # Prepare the input
    inp = test_data["Li"]
