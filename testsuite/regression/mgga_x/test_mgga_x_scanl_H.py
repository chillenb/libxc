...
def test_mgga_x_scanl_H_2_zk():
    ...
    tgt = out["zk"].flatten()
    ref_tgt = [-5.929714835837920e-01, -6.015312376939183e-01, -3.804608272296274e-01, -9.699644226134350e-02, -1.741036439912599e-03]
...
def test_mgga_x_scanl_H_2_vrho():
    ...
    ref_tgt = [-8.604714652010326e-01, -6.868878405698020e-17, -6.797670547280916e-01, -2.287824955618894e-16, -4.776107594407467e-01, -4.631151438487701e-17, -1.396733957581818e-01, -4.321935761763037e-17, -3.171480118500463e-03, 5.721564036697639e-20]
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05
...
def test_mgga_x_scanl_H_2_vlapl():
    ...
    ref_tgt = [6.384774633606762e-03, 0.000000000000000e+00, 1.652320648794779e-02, 0.000000000000000e+00, 4.751167703402203e-03, 0.000000000000000e+00, 0.000000000000000e+00, 0.000000000000000e+00, 5.907203238993317e-05, 0.000000000000000e+00]
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05


def test_mgga_x_scanl_H_2_vtau():
    # Prepare the input
    inp = test_data["H"]
