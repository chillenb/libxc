...
def test_mgga_x_msb86bl_H_2_vrho():
    ...
    ref_tgt = [-8.957542043460383e-01, -8.833398646122426e-17, -7.907448763018889e-01, -1.555980780163193e-16, -4.494152642250065e-01, -2.734619578389743e-17, -1.156694919921859e-01, -8.749891659586644e-17, -1.523708977953147e-02, -2.301188979799802e-18]
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05
...
def test_mgga_x_msb86bl_H_2_vsigma():
    ...
    ref_tgt = [-8.039066051733715e-03, 0.000000000000000e+00, 0.000000000000000e+00, -1.199033080317941e-02, 0.000000000000000e+00, 0.000000000000000e+00, -9.500746767059463e-02, 0.000000000000000e+00, 0.000000000000000e+00, -7.012088651217539e+00, 0.000000000000000e+00, 0.000000000000000e+00, -5.251251783982146e+03, 0.000000000000000e+00, 0.000000000000000e+00]
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05


def test_mgga_x_msb86bl_H_2_vlapl():
    # Prepare the input
    inp = test_data["H"]
...
def test_mgga_x_msb86bl_H_2_vtau():
    ...
    ref_tgt = [1.151846855677654e-12, 0.000000000000000e+00, 2.444569619433950e-11, 0.000000000000000e+00, 8.006261155831794e-11, 0.000000000000000e+00, -4.326414693093551e-16, 0.000000000000000e+00, 2.141063824778525e-08, 0.000000000000000e+00]
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05
