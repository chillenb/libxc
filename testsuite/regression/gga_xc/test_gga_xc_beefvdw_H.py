...
def test_gga_xc_beefvdw_H_2_zk():
    ...
    tgt = out["zk"].flatten()
    ref_tgt = [-6.751428613983055e-01, -6.185019960213308e-01, -3.900434929410682e-01, -1.555484171804739e-01, -8.609630249572399e-03]
...
def test_gga_xc_beefvdw_H_2_vrho():
    ...
    ref_tgt = [-8.929507413333772e-01, 4.808581348129403e-01, -7.784776752948900e-01, 2.938717684953902e+01, -4.232814128721532e-01, 1.643401247952471e+01, -1.755103951536400e-01, 8.093721243510245e-02, -1.142005915717980e-02, -4.243940879855832e-03]
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05
