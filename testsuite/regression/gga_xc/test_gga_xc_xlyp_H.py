...
def test_gga_xc_xlyp_H_2_zk():
    ...
    tgt = out["zk"].flatten()
    ref_tgt = numpy.asarray([-6.220990966746058e-01, -5.817524021815097e-01, -3.637781870927487e-01, -1.475302668221355e-01, -4.269815100018623e-02])
...
def test_gga_xc_xlyp_H_2_vrho():
    ...
    ref_tgt = numpy.asarray([-8.281424839700010e-01, -2.361713832519182e-01, -7.172576115675632e-01, -2.499095375018286e-01, -4.029809246578913e-01, -1.966669686227086e-01, -1.148626090675543e-01, -3.555089138121431e-02, -1.066946718392586e-02, -2.453539843799461e-03])
    error = numpy.max(numpy.abs(tgt-ref_tgt))/(1.0+numpy.max([numpy.abs(tgt), numpy.abs(ref_tgt)]))
    assert error < 5e-05
